...
function resolveProject(
  ...
  if (projectIds.length === 0) {
    throw new Error("No projects configured. Add a project to syntese.yaml.");
  }
...
async function handleUrlStart(
  ...
  for (const filename of CONFIG_CANDIDATES) {
    const configPath = resolve(targetDir, filename);
  ...
  const configPath = resolve(targetDir, "syntese.yaml");
...
async function runStartup(
  ...
  if (opts?.dashboard !== false) {
    ...
    if (!existsSync(resolve(webDir, "package.json"))) {
      throw new Error("Could not find @syntese/web package. Run: pnpm install");
    }
